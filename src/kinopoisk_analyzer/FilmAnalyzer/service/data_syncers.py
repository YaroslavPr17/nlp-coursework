...
from src.kinopoisk_analyzer.utils.requests import FiltersRequester
from src.kinopoisk_analyzer.FilmAnalyzer.utils.constants import project_path, data_path
from src.kinopoisk_analyzer.utils.stylifiers import Stylers
import logging

package_path = Path(project_path, 'src', 'kinopoisk_analyzer', 'FilmAnalyzer', 'service')


def sync_everything():
    sync_genres()
    sync_countries()
    sync_countries_coordinates()

    print('Syncing finished.')


def sync_genres():
    print('Synchronizing genres list from Kinopoisk...', end=' | ')
    ...
    print(Stylers.bold('Finished!'))

def sync_countries():
    print('Synchronizing countries list from Kinopoisk...', end=' | ')
...
def sync_countries_coordinates():
    print('Synchronizing countries\' coordinates from repository...')
    ...
    _tqdm_range = tqdm.tqdm(countries_files_names, file=sys.stdout)
    _tqdm_range.set_description(f'Downloading countries\' border-coordinates')

    n_retrieved = 0
    n_saved = 0
    for country_file_name in _tqdm_range:
